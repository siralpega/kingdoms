...
public class SignEditor {
    ...
    public SignEditor(MinecraftReflector reflector) {
    ...
    public void commit(Player player, Sign sign)
    {
        ...
        try {
            ...
            formatSignForSave(sign);
            player.sendMessage("§c§lFailed to invoke sign editor!");
            player.sendMessage("§7Likely cause: §rMinecraft server API changed");
            player.sendMessage("§7Server admin: §rCheck for updates to this plugin");
    
            player.sendMessage("§7Error code: §r" + e.toString());
            player.sendMessage("§6(More details logged in server console)");
